...
def main() -> None:
    ...
    i = 1
    while i <= expected_length and secret != guess:
        # User can play while they have guesses left AND they have not yet won
        print(f"=== Turn {i}/{expected_length} ===")
        guess = input_guess(expected_length)
        print(emojified(guess, secret))
        i = i + 1
    if secret == guess:
        print(f"You won in {i-1}/{expected_length} turns!")
        # The guess word is the same as the secret word - Yay! Win!
    else:
        print(f"X/{expected_length} - Sorry, try again tomorrow!")
